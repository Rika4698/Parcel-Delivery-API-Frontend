...
export default function TrackParcel() {
  ...
  const { data: TrackedData, isLoading, isError } = useGetParcelByTrackingIdQuery(
    submittedId!,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-purple-50 to-blue-100 dark:from-gray-900 dark:via-slate-800 dark:to-gray-900 py-32 sm:py-40">
      <div className="container mx-auto px-4">
        ...
        {/* Loading */}
        { isLoading && submittedId && trackingId.trim() && (
            ...
            <Card className="bg-white/70 dark:bg-gray-900/60 border border-purple-200 dark:border-gray-700 shadow-md backdrop-blur-md">
              <CardContent className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
                  ...
                  <div>
                    <h3 className="text-sm text-gray-500 dark:text-gray-400">Weight</h3>
                    <p className="font-bold text-gray-800 dark:text-gray-200">
                      {trackingResult?.parcelDetails?.weight} kg
                    </p>
                  </div>
                  <div>
                    <h3 className="text-sm text-gray-500 dark:text-gray-400">Note</h3>
                    <p className="font-bold text-gray-800 dark:text-gray-200">
                      {trackingResult?.parcelDetails?.note || "—"}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-sm text-gray-500 dark:text-gray-400">Sender</h3>
                    <p className="font-bold text-gray-800 dark:text-gray-200">
                  ...
                  <div>
                    <h3 className="text-sm text-gray-500 dark:text-gray-400">Receiver</h3>
                    <p className="font-bold text-gray-800 dark:text-gray-200">
            ...
            <div className="bg-gradient-to-br from-blue-100 to-purple-100 dark:from-slate-800 dark:to-gray-900 border border-blue-200 dark:border-gray-700 p-8 rounded-2xl shadow-inner">
              ...
              <ul className="space-y-8 max-w-3xl mx-auto">
                {trackingResult?.statusHistory.map((item, idx) => {
                  const Icon = statusIcons[item.status] || statusIcons.DEFAULT;
                  return (
                    <li key={idx} className="relative flex items-start gap-6">
                      {idx !== trackingResult?.statusHistory?.length - 1 && (
